...
def get_user_stats(username: str = Header(alias="X-Username")):
    ...
    # Get total answered: prefer stats file, then fallback to practice files
...
def record_answer(data: dict):
    ...
    # Also increment user's total answered count
...
# Static files - serve everything from the quiz-app directory
# index.html at root
@app.get("/")
def serve_index():
    return FileResponse(STATIC_DIR / "index.html", media_type="text/html")

# Mount static files for CSS, JS, JSON etc.
